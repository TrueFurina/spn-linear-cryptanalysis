...
# Test ONE case with various topk
u, v, R, vt = 0x01000000, 0x10000000, 5, -2.06493e-5
...
for k in [3, 6, 10]:
    ...
    for ri in range(R):
        ...
        for m,c in state.items():
            results = fwd_one_topk(m, k)
            n_ops += len(results)
            for nm,nc in results:
                ns[nm] += c*nc
        n_masks = len(ns)
        print(f"  Round {ri+1}: {len(state)} masks in, {n_ops} trails, {n_masks} masks out")
        if n_masks > 2000:
            items = sorted(ns.items(), key=lambda x: abs(x[1]), reverse=True)
            state = dict(items[:2000])
        else:
            state = dict(ns)
    ve = state.get(v, 0.0)
    ratio = ve/vt if vt != 0 else float('inf')
    ok = 0.75 <= ratio <= 1.25
    elapsed = time.time()-t0
    print(f"  V_E={ve:.8e} V_T={vt:.8e} ratio={ratio:.4f} {'OK' if ok else 'MISS'} ({elapsed:.2f}s)")
