...
### V_T 精确性的独立校验（`src/verify_vt_exactness.py`）

轮次 ≥5 时 DP 状态爆炸，无法逐条重算，因此改用**不依赖方式 1 计算的数学可证伪检验**。
穷举定义下 `S = Σ_x (−1)^(u·x ⊕ v·F^r(x))` 为整数、`V_T = S/2³²`，故必有两条必要条件：

| 条件 | 判据 | 实测结果 |
|---|---|---|
| (A) 整数性 | `V_T·2³² ∈ Z` | 346/346 通过 |
| (B) 奇偶性 | `S = 2³² − 2·#odd` 必为偶数 | 346/346 通过 |

最大取整偏差 0.214，仅由 5~6 位有效数字打印造成。若 `V_T` 为任意估计实数，单条随机
落在含整数区间内的概率约 0.4，346 条全通过的概率 `≈ 0.4³⁴⁶ ≈ 0`，故该检验具统计决定性：
**全部 V_T 可判定为 2³² 穷举精确值，而非估计/采样值**。评委可独立运行该脚本复核。
...
└── src/                          # 核心算法实现
    ...
    ├── verify_vt_exactness.py    #  ★ V_T 精确性独立校验（不依赖方式1的可证伪检验）
